...
public class Route extends JInternalFrame {
	...
	public Route() {
		...
		for(Vehicle temp : SysData.getInstance().getVehicles()) {
			System.out.println(temp.getVin() + temp.isInUse() + temp.getParcels().size());
		...
		refresh.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for(Map.Entry<String, Vehicle > temp : SysData.getInstance().getVehclesMap().entrySet()) {
					if(temp.getValue() instanceof Truck && temp.getValue().isInUse() && temp.getValue().getDriver() != null && temp.getValue().getDriver().getWareHousesToGo().isEmpty()) {
						Truck tr = (Truck) temp.getValue();
						System.out.println(temp.getValue());
						System.out.println(temp.getValue().getDriver().getWareHousesToGo().isEmpty());
		...
		addRoute.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(table_1.getSelectedRow() < 0 || table_2.getSelectedRow() < 0) {
					...
					WareHouse wh1 = SysData.getInstance().getWareHouseById((int)table_1.getValueAt(table_1.getSelectedRow(), 0));
					WareHouse wh2 =	SysData.getInstance().getWareHouseById((int)table_2.getValueAt(table_2.getSelectedRow(), 0));
//					array = (ArrayList<Parcel>)(SysData.getInstance().addAllPossibleParcelsToTruck((String)tr1.getVin(), (int)table_1.getValueAt(table_1.getSelectedRow(), 0), (int)table_2.getValueAt(table_2.getSelectedRow(), 0)));
					...
					driv.getWareHousesToGo().put(wh1, wh2.getWarehouseId());
					...
					model_1.setRowCount(0);
					for(WareHouse wh: SysData.getInstance().WareHouses()) {
						if(!wh.getParcels().isEmpty() && !driv.getWareHousesToGo().containsKey(wh)) {
							rowData_1[0] = wh.getWarehouseId();
							model_1.addRow(rowData_1);
						}
					}
					if(driv.getWareHousesToGo().size() == 4 || table_1.getModel().getRowCount() == 0)
						refresh.doClick();
